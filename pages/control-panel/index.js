...
import { createTile, createTiles, deleteTile, getTiles, updateTile } from "../../services/utils";
...
export default function ControlPanelPage() {
	...
	function handleAppendNewTile(e) {
		...
		if ((tab === "south" || tab === "north") && showUpper) {
			return setError(`Cannot Add Tile Where block = ${tab} and section = Upper`)
		}
	...
	function check(tile) {
		...
		for (let i = 0; i < newTiles.length; i++) {
			if (newTiles[i].row === tile.row && newTiles[i].col === tile.col && newTiles[i].id !== tile.id) {
				setError(`Found Duplicate in New Tiles row ${tile.row} col ${tile.col} (${tile.name}) collides with (${newTiles[i].name})`)
				return false
			}
		}
        ...
        <div className="h-screen">
            <nav className="flex w-full h-15 shadow-md text-xl mobile:text-5xl">
                ...
                <div className="w-full flex flex-row justify-center items-center top-0">
                    ...
                    <Button onClick={() => setShowEdit(!showEdit)}>{!showEdit ? "Edit" : "Preview"}</Button>
                    {showEdit && (
                        <>
                            <Button
                                onClick={handleAppendNewTile}
                            >
                                Add Tile
                            </Button>
							{newTiles.length > 1 && <Button onClick={handleAddAll}>
								Add All
							</Button>}
                            <Link
                ...
                {showEdit && showUpper && (upperTiles.length > 0 || newTiles.length > 0) && <MiniSpreadSheet newTiles={newTiles} sectionTiles={upperTiles} handleAdd={handleAdd} handleEdit={handleEdit} handleChange={handleChange} handleDelete={handleDelete}/>}
				{showEdit && !showUpper && (lowerTiles.length > 0 || newTiles.length > 0) && <MiniSpreadSheet newTiles={newTiles} sectionTiles={lowerTiles} handleAdd={handleAdd} handleEdit={handleEdit} handleChange={handleChange} handleDelete={handleDelete}/>}
		</div>
    );
}
