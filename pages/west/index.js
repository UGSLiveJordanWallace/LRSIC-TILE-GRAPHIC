...
export default function WestBlockPage() {
	...
	useEffect(() => {
		getTiles(setUpperTiles, setLowerTiles, "west");
	}, [])

	function handleSearch(e) {
		e.preventDefault();
		searchPavers(e.target.value, searchType, sectRender, setSearchResults, upperTiles, lowerTiles, setError);
	}

	async function findPaverLocation(e, paver) {
		e.preventDefault();
		locatePaverCoords(sectRender, paver, upperTiles, lowerTiles, setSearchRender);
	}
